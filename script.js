...
function loadQuestion() {
  ...
  // BUG 1: Off-by-one — allows questionNumber = 0, making questions[-1] = undefined
  // Fix should be: questionNumber < 1, not questionNumber < 0
...
function showQuestion() {
  ...
  // BUG 2: Question counter displays wrong number — shows index instead of 1-based number
  // currentQuestionIndex is 0-based, so Q1 shows "Question 0"
  questionTitle.innerText = `Question ${currentQuestionIndex}`;
  ...
  currentQuestion.options.forEach(option => {
    ...
    // BUG 3: Event listener is added but never removed — if showQuestion() is called
    // multiple times on the same container without clearing it first, listeners stack.
    // answersContainer.innerHTML = '' removes the DOM nodes but this pattern is fragile
    // when combined with delegated handlers or re-renders without full resets.
...
function handleAnswer(selectedAnswer, currentQuestion) {
  // BUG 4: No guard against multiple clicks — user can click two buttons
  // before the alert closes, calling handleAnswer twice and double-counting score
  // or skipping two questions at once. No `answered` flag is set.
...
function endGame() {
  // BUG 5: finalScore shows raw score number but never formats or validates it.
  // If score is NaN (e.g. from a corrupted currentQuestionIndex), it displays "NaN"
  // with no fallback. Should use: finalScore.innerText = isNaN(score) ? 0 : score;
